...
/**
  * Created by frank on 11/07/16.
  */
object EnronTimeGraph extends App{
  ...
  sentMails.collect().map( mail => {
      // Date line
      val dateLine = mail.split("\n").filter(line => line.contains("Date: ")).head
      val date = new Date(dateLine.substring(6).trim)
    (mail,date)
  }
    )
      .sortBy(_._2)
  ...
  println("\n Accuracy of the recommender time sorted : "+(correctReco.toLong/testSetSize.toLong).toLong+" with : "+correctReco+" correct guesses on "+totalGroupMail+" recipients in test set")
