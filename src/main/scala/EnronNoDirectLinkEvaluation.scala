...
/**
  * Created by frank on 08/07/16.
  *
  * This class aims at creating at evaluating the graph without direct links between users
  */
object EnronNoDirectLinkEvaluation extends App{
  ...
  val sc = new SparkContext(new SparkConf()
    ...
    .setMaster("local[8]")
    .setAppName("EnronNoDirectLinkGraphEvaluation")
  )
  ...
  sentMails.collect().foreach(f = mail => {
    ...
    //  create an anonymous node and make a link from-node, node-tos
    if(toArray.length>1){
