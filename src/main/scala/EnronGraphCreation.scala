...
object EnronGraphCreation extends App{
  ...
  val usersRDD: RDD[(VertexId, (String))] = sc.parallelize(users.map(mail => (users.indexOf(mail),users(users.indexOf(mail)))).toArray)
  ...
  // ShortestPaths to
  val currentUser = users.indexOf("tim.belden@enron.com")
  val dest = users.indexOf("phillip.allen@enron.com")
